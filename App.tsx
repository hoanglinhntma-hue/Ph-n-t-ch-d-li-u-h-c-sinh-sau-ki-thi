...
const App: React.FC = () => {
  ...
  // Tính điểm trung bình môn học và sắp xếp từ thấp đến cao
  const subjectAverages = useMemo(() => {
    ...
    currentViewStudents.forEach(s => {
      s.scores.forEach(scoreObj => {
        if (!subjectStats[scoreObj.name]) {
          subjectStats[scoreObj.name] = { total: 0, count: 0 };
        }
        if (scoreObj.score >= 0) {
  ...
  const exportToExcelFormatted = () => {
    ...
    ws['!cols'] = [{ wch: 5 }, { wch: 25 }, { wch: 10 }, { wch: 20 }, ...headers.map(() => ({ wch: 8 })), { wch: 50 }];
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col" translate="no">
              ...
              <div className="relative border-4 border-double border-slate-200 rounded-[32px] p-8">
                <div className="flex justify-between items-start mb-8">
                  <div className="flex items-center gap-3">
                    <div className="p-3 bg-indigo-600 rounded-2xl shadow-lg shadow-indigo-200"><Trophy className="w-8 h-8 text-white" /></div>
                    <div>
                ...
                <div className="mb-10">
                  ...
                  <div className="mt-3 flex items-center gap-2">
                    <span className={`px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-wider ${getClassificationStyles(selectedStudentForCard.classification)}`}>{selectedStudentForCard.classification}</span>
                    <div className="h-px flex-1 bg-slate-100"></div>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-8 mb-8">
                  <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100">
                    <h5 className="flex items-center gap-2 text-xs font-black text-slate-400 uppercase mb-4"><Target className="w-4 h-4 text-rose-500" /> Mục tiêu bứt phá</h5>
                    <div className="space-y-3">
                      {selectedStudentForCard.goals.length > 0 ? selectedStudentForCard.goals.map((g, i) => (
                        ...
                        <div className="flex items-center gap-3 text-emerald-600"><Star className="w-4 h-4 fill-emerald-600" /><span className="text-sm font-black uppercase">Duy trì phong độ Tốt</span></div>
                      )}
                    </div>
                  </div>
                  <div className="bg-indigo-600 p-6 rounded-3xl text-white shadow-xl shadow-indigo-100 relative overflow-hidden">
                    ...
                    <p className="text-sm font-medium leading-relaxed italic relative z-10">"Sự kiên trì là chìa khóa của thành công. Hãy tập trung vào mục tiêu để bứt phá!"</p>
                  </div>
                </div>
                <div className="pt-6 border-t border-slate-100 flex justify-between items-center">
                  <div className="flex items-center gap-2"><Sparkles className="w-4 h-4 text-amber-400" /><p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest italic">Success is a journey, not a destination</p></div>
                  <div className="w-12 h-12 bg-slate-50 rounded-xl flex items-center justify-center border-2 border-slate-100"><Award className="w-6 h-6 text-slate-200" /></div>
                </div>
              </div>
            </div>
            <div className="p-8 bg-slate-50 flex gap-4">
              <button onClick={downloadStudentCard} disabled={exporting} className="flex-1 py-4 bg-indigo-600 text-white rounded-2xl font-black text-sm flex items-center justify-center gap-2 hover:bg-indigo-700 transition shadow-xl shadow-indigo-100 disabled:opacity-50">
                {exporting ? <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div> : <ImageIcon className="w-5 h-5" />} Tải Thẻ Ảnh Mục Tiêu
              </button>
              <button onClick={() => setSelectedStudentForCard(null)} className="px-8 py-4 bg-white text-slate-600 rounded-2xl font-black text-sm border border-slate-200 hover:bg-slate-50 transition">Đóng</button>
            </div>
          </div>
        </div>
      )}
      ...
      <header className="bg-white border-b sticky top-0 z-50 no-print">
        <div className="max-w-[1600px] mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-indigo-600 rounded-xl shadow-lg shadow-indigo-200"><Sparkles className="w-5 h-5 text-white" /></div>
            <div>
          ...
          <div className="flex items-center gap-2">
            <button onClick={exportToExcelFormatted} className="flex items-center gap-2 px-4 py-2 bg-white text-slate-700 rounded-xl hover:bg-slate-50 transition border border-slate-200 shadow-sm text-sm font-bold no-print"><FileSpreadsheet className="w-4 h-4 text-emerald-600" /> <span>Xuất Excel Đẹp</span></button>
            ...
            {allStudents.length > 0 && (
              <button onClick={() => { if(window.confirm("Xóa sạch dữ liệu?")) { setAllStudents([]); setHeaders([]); setActiveTab("SUMMARY"); } }} className="p-2 text-rose-500 hover:bg-rose-50 rounded-lg transition-colors"><Trash2 className="w-5 h-5" /></button>
            )}
          </div>
        </div>
      </header>
        ...
        <aside className="w-64 border-r bg-white hidden lg:flex flex-col p-4 sticky top-16 h-[calc(100vh-64px)] overflow-y-auto no-print">
          ...
          <div className="space-y-1">
            ...
            {sortedClassNames.map(cls => (
              <button key={cls} onClick={() => setActiveTab(cls)} className={`w-full flex items-center justify-between px-4 py-2.5 rounded-lg transition-all text-sm font-bold ${activeTab === cls ? 'bg-indigo-50 text-indigo-700 border-l-4 border-indigo-600' : 'text-slate-500 hover:bg-slate-50'}`}><span className="truncate">{cls}</span><span className={`text-[10px] px-1.5 py-0.5 rounded-md ${activeTab === cls ? 'bg-indigo-200/50' : 'bg-slate-100'}`}>{classGroups[cls].length}</span></button>
            ))}
          </div>
        </aside>
          ...
          {allStudents.length === 0 ? (
            ...
            <div className="space-y-8 pedagogical-report" ref={reportRef}>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                {/* THAY THẾ CỐ VẤN AI BẰNG ĐIỂM TRUNG BÌNH MÔN */}
                <div className="bg-white p-8 rounded-[32px] shadow-sm border border-slate-100 overflow-hidden flex flex-col">
                  <div className="flex items-center justify-between mb-6">
                    <h3 className="font-black text-slate-900 flex items-center gap-2"><BookOpen className="w-5 h-5 text-indigo-600" /> Điểm TB Môn học</h3>
                    <TrendingDown className="w-4 h-4 text-rose-500" />
                  </div>
                  <div className="flex-1 overflow-y-auto pr-2 space-y-4 max-h-[300px] custom-scrollbar">
                    {subjectAverages.length > 0 ? subjectAverages.map((sub, idx) => (
                      <div key={sub.name} className="space-y-1.5 group">
                        <div className="flex justify-between items-center text-xs">
                          <span className="font-bold text-slate-700 flex items-center gap-2">
                            {idx < 2 && <Target className="w-3 h-3 text-rose-500" />}
                            {sub.name}
                          </span>
                          <span className={`font-black ${sub.average < 5 ? 'text-rose-500' : 'text-slate-900'}`}>{sub.average.toFixed(2)}</span>
                        </div>
                        <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden">
                          <div 
                            className={`h-full transition-all duration-1000 ${getAverageColor(sub.average)}`} 
                            style={{ width: `${(sub.average / 10) * 100}%` }}
                          ></div>
                        </div>
                        {idx < 2 && <p className="text-[9px] text-rose-400 font-bold uppercase italic tracking-tighter">Ưu tiên cải thiện</p>}
                      </div>
                    )) : (
                      <div className="h-full flex flex-col items-center justify-center text-slate-300 py-10">
                        <BarChart3 className="w-10 h-10 mb-2 opacity-20" />
                        <p className="text-[10px] font-bold">Chưa có dữ liệu môn học</p>
                      </div>
                    )}
                  </div>
                  <div className="mt-6 pt-4 border-t border-slate-50">
                    <p className="text-[10px] text-slate-400 font-medium italic">* Danh sách được sắp xếp từ điểm thấp đến cao để nhận diện môn học yếu điểm.</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-[40px] shadow-sm border border-slate-100 overflow-hidden" style={{fontFamily: "'Times New Roman', Times, serif", fontSize: "16pt"}}>
                <div className="p-8 border-b flex flex-col md:flex-row md:items-center justify-between gap-6">
                  ...
                  <div className="flex gap-3 no-print">
                    ...
                    <select className="px-6 py-3 bg-slate-50 border-none rounded-2xl text-sm font-bold text-slate-600 appearance-none shadow-sm" value={selectedClassification} onChange={(e) => setSelectedClassification(e.target.value)}>
                      ...
                      {[...filteredStudents].sort((a,b) => (classificationPriority[a.classification] || 99) - (classificationPriority[b.classification] || 99)).map(s => (
                        <tr key={`${s.className}-${s.id}-${s.name}`} className="hover:bg-slate-50/50 transition-colors group">
                              ...
                              {s.goals.length > 0 ? (
                                <div className="flex flex-col gap-1">
                                  {s.goals.slice(0, 1).map((g, i) => (
                                    <div key={i} className="flex items-center gap-2"><span className="font-black text-slate-700">{g.subjectName}</span><ChevronRight className="w-3 h-3 text-indigo-400" /><span className="text-emerald-600 font-black">{g.targetScore.toFixed(1)}</span></div>
                                  ))}
                                </div>
                          ...
                          <td className="px-8 py-6 text-right no-print">
                            <button onClick={() => setSelectedStudentForCard(s)} className="inline-flex items-center gap-2 px-4 py-2 bg-indigo-50 text-indigo-600 rounded-xl hover:bg-indigo-600 hover:text-white transition-all font-black text-xs border border-indigo-100"><Award className="w-4 h-4" /> Thẻ Mục Tiêu</button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
